...
async def _(client, message):
    ...
    # Ekstrak ID user dan durasi (Contoh: .prem @user 30d / 1m / permanent)
    user_id, duration = await extract_user_and_reason(message)
    msg = await message.reply("⏳ ᴍᴇᴍᴘʀᴏsᴇs...")
    
    if not user_id:
        return await msg.edit(
            f"<b>Format: {message.text} [ID/Username] [Durasi]</b>\n\n"
            "<b>Pilihan Durasi:</b>\n"
            "- <code>30d</code> (30 Hari)\n"
            "- <code>1m</code> (1 Bulan)\n"
            "- <code>permanent</code> (Selamanya)"
        )
    ...
    # Logika Penentuan Masa Aktif
    if not duration:
        # Default jika tidak isi durasi: 1 bulan
        expired = now + relativedelta(months=1)
        durasi_text = "1 Bulan"
    elif duration.lower() == "permanent":
        # Permanen diset 100 tahun
        ...
        try:
            ...
            durasi_text = f"{days} Hari"
        except:
            return await msg.edit("Format Hari Salah! Contoh: 7d")
    elif duration.endswith("m"):
        try:
            months = int(duration.replace("m", ""))
            expired = now + relativedelta(months=months)
            durasi_text = f"{months} Bulan"
        except:
            return await msg.edit("Format Bulan Salah! Contoh: 1m")
    else:
        # Jika hanya angka, anggap bulan
        try:
            months = int(duration)
            expired = now + relativedelta(months=months)
            durasi_text = f"{months} Bulan"
        except:
            return await msg.edit("Durasi tidak valid!")
    ...
    try:
        ...
        # Pesan Sukses
        await msg.edit(f"""
<blockquote><b>ɴᴀᴍᴇ: [{target_user.first_name} {target_user.last_name or ''}](tg://user?id={target_user.id})</b>
<b>ɪᴅ: `{target_user.id}`</b>
<b>ᴇxᴘɪʀᴇᴅ: {durasi_text}</b>
<b>ꜱɪʟᴀʜᴋᴀɴ ʙᴜᴋᴀ @biiew_bot ᴜɴᴛᴜᴋ ᴍᴇᴍʙᴜᴀᴛ ᴜꜱᴇʀʙᴏᴛ</b></blockquote>

<blockquote>ᴄᴀʀᴀ ʙᴜᴀᴛ ᴜsᴇʀʙᴏᴛ :
- sɪʟᴀʜᴋᴀɴ /start ᴅᴜʟᴜ ʙᴏᴛ @biiew_bot 
- ᴋᴀʟᴀᴜ sᴜᴅᴀʜ sᴛᴀʀᴛ ʙᴏᴛ ᴀʙɪsᴛᴜ ᴘᴇɴᴄᴇᴛ ᴛᴏᴍʙᴏʟ ʙᴜᴀᴛ ᴜsᴇʀʙᴏᴛ</blockquote>
""")
        ...
        await bot.send_message(
            OWNER_ID,
            f"• ɪᴅ-ꜱᴇʟʟᴇʀ: `{user.id}`\n"
            f"• ɪᴅ-ᴄᴜꜱᴛᴏᴍᴇʀ: `{target_user.id}`\n"
            f"• ᴅᴜʀᴀsɪ: `{durasi_text}`",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("👑 sᴇʟʟᴇʀ", callback_data=f"profil {user.id}"),
                InlineKeyboardButton("👤 ᴄᴜsᴛᴏᴍᴇʀ", callback_data=f"profil {target_user.id}")
            ]]),
        )
    except Exception as error:
        await msg.edit(f"Gagal: {str(error)}")
